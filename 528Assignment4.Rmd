...
inputFilePath <- ("C:/path/to/your/file/E_coli_K12_MG1655_.400_50")
outputFilePath <- "C:/path/to/your/file/E_coli_K12.fasta"
# Read the raw data
...
scoreSequenceWithPSWM <- function(sequence, pswm) {
  ...
  for (pos in 1:length(scores)) {
    subseq <- substring(sequence, pos, pos + ncol(pswm) - 1)
    tempScore <- 0  # Initialize a temporary score variable for this position
    for (i in 1:ncol(pswm)) {
      nucleotide <- substr(subseq, i, i)
      # Add the score for this nucleotide at this position
      if(nucleotide %in% rownames(pswm)) {
...
scoresList <- t(data.frame(lapply(E.coli, scoreSequenceWithPSWM, pswm=weights)))
